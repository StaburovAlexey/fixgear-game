...
setup_pm2_for_project() {
  ...
  # Перед запросом entry-файла спрашиваем про сборку сервера
  local DEFAULT_BUILD_CMD="npm run build"
  read -rp "${COLOR_ASK}Нужно ли собрать сервер перед запуском? (y/n, default: y): ${COLOR_RESET}" NEED_BUILD
  NEED_BUILD="${NEED_BUILD:-y}"
  if [ "$NEED_BUILD" = "y" ] || [ "$NEED_BUILD" = "Y" ]; then
    read -rp "${COLOR_ASK}Команда сборки (по умолчанию ${DEFAULT_BUILD_CMD}): ${COLOR_RESET}" BUILD_CMD
    BUILD_CMD="${BUILD_CMD:-$DEFAULT_BUILD_CMD}"
    echo ">>> Выполняю сборку: $BUILD_CMD"
    bash -lc "$BUILD_CMD"
  else
    echo ">>> Сборка пропущена по запросу пользователя."
  fi
